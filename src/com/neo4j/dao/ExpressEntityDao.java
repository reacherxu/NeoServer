...
public class ExpressEntityDao extends BaseDao {
	...
	public List<ExpressEntity> getAllExpressEntity() {
		...
		for (int i = 0; i < entities.size(); i++) {
			entityList
					.add(getExpressEntity((Integer) entities.get(i).get("id")));
		}
	...
	/**
	 * 根据entity的名字返回对应的entity 若图中无entity，返回null
	 * 
	 * @param name
	 * @return
	 */
	...
	private List<String> getBase(Integer entity_decl) {
		...
		if (getIdByName(subsuper, "subtype_declaration").size() != 0) {
			Integer subtype_declaration = getIdByName(subsuper,
					"subtype_declaration").get(0);
			List<Integer> entity_refs = getIdByName(subtype_declaration,
					"entity_ref");
			/* 添加所有的父类 */
			for (int i = 0; i < entity_refs.size(); i++) {
				String sql = "start n=node({1}) match (n:Node)-[r*2..2]->(m:Node) return m.name as name";
				bases.add((String) this.getNeoConn()
						.query(sql, entity_refs.get(i)).get("name"));
			}
		}
		return bases;
	}
	...
	public ExpressEntity getExpressEntity(Integer entity_decl) {
		...
		List<Map<String, Object>> explicit_attrs = this.getNeoConn().queryList(
				sql, entity_decl, new String("explicit_attr"));
		List<Map<String, Object>> derive_clauses = this.getNeoConn().queryList(
				sql, entity_decl, new String("derive_clause"));
		List<Map<String, Object>> inverse_clauses = this.getNeoConn()
				.queryList(sql, entity_decl, new String("inverse_clause"));
		List<Map<String, Object>> unique_clauses = this.getNeoConn().queryList(
				sql, entity_decl, new String("unique_clause"));
	 ...
	 * @return
	 */
	private Map<GeneralizedInstance, List<String>> getExpressExplicitInstance(
			List<Map<String, Object>> explicit_attrs) {
	...
	private Map<? extends GeneralizedInstance, ? extends List<String>> getExpressDeriveInstance(
		...
		for (int i = 0; i < derive_clauses.size(); i++) {
			...
			String sql = "start n=node({1}) match (n:Node)-[*1..]->(m:Node) where m.name={2} return ID(m) as id";
			List<Map<String, Object>> derived_attrs = this.getNeoConn()
					.queryList(sql, derive_clause, new String("derived_attr"));
			for (int j = 0; j < derived_attrs.size(); j++) {
				...
				/* 获取expression */
				Integer expression = getIdByName(derived_attr, "expression")
						.get(0);
				String simple_expression = (String) getDirectChildren(
						expression).get(0).get("name");
	 ...
	 * @return
	 */
	private Map<? extends GeneralizedInstance, ? extends List<String>> getExpressInverseInstance(
			List<Map<String, Object>> inverse_clauses) {
		Map<GeneralizedInstance, List<String>> instances = new HashMap<GeneralizedInstance, List<String>>();
		...
		for (int i = 0; i < inverse_clauses.size(); i++) {
			...
			String sql = "start n=node({1}) match (n:Node)-[*1..]->(m:Node) where m.name={2} return ID(m) as id";
			List<Map<String, Object>> inverse_attrs = this.getNeoConn()
					.queryList(sql, inverse_clause, new String("inverse_attr"));
			for (int j = 0; j < inverse_attrs.size(); j++) {
				Integer inverse_attr = (Integer) inverse_attrs.get(j).get("id");

				Integer entity_ref = getIdByName(inverse_attr, "entity_ref")
						.get(0);
				Integer attribute_ref = getIdByName(inverse_attr,
						"attribute_ref").get(0);
				List<Map<String, Object>> tmpVars = getVariables(inverse_attr);

				if (hasDirectChild(inverse_attr, "OF")) {
					Integer bound_spec = getIdByName(inverse_attr, "bound_spec")
							.get(0);

					if (hasDirectChild(inverse_attr, "SET")) {

						/* new Express Set instance */
						Integer SET = (Integer) getDirectChildren(inverse_attr)
								.get(0).get("id");
						...
						ExpressSet tmpSet = new ExpressSet(SET, bounds[0],
								bounds[1], innerEntity);

						for (int k = 0; k < tmpVars.size(); k++) {
							SetInstance setIns = new SetInstance(
									(Integer) tmpVars.get(k).get("id"),
									(String) tmpVars.get(i).get("name"), tmpSet);

							List<String> tmpList = new ArrayList<String>();
							tmpList.add("INVERSE:" + innerEntity.getName()
									+ " FOR " + getLeaf(attribute_ref));

							instances.put(setIns, tmpList);
						}
					} else {
						/* new Express Bag instance */
						Integer BAG = (Integer) getDirectChildren(inverse_attr)
								.get(0).get("id");
						ExpressBagDao bagDao = new ExpressBagDao();
						Integer[] bounds = bagDao.getBound(bound_spec);
						ExpressEntity innerEntity = (ExpressEntity) getEntityRef(entity_ref);
						ExpressBag tmpBag = new ExpressBag(BAG, bounds[0],
								bounds[1], innerEntity);

						for (int k = 0; k < tmpVars.size(); k++) {
							BagInstance bagIns = new BagInstance(
									(Integer) tmpVars.get(k).get("id"),
									(String) tmpVars.get(i).get("name"), tmpBag);
	 ...
	 * @return
	 */
	private Map<String, List<String>> getExpressUniqueInstance(
			List<Map<String, Object>> unique_clauses) {
		...
		for (int i = 0; i < unique_clauses.size(); i++) {
			...
			List<Integer> unique_rules = getIdByName(unique_clause,
					"unique_rule");
			...
			for (int j = 0; j < unique_rules.size(); j++) {
				...
				Integer rule_label_id = (Integer) getDirectChildren(unique_rule)
						.get(0).get("id");
				ruleName = (String) getDirectChildren(rule_label_id).get(0)
						.get("name");

				List<Integer> referenced_attributes = getIdByName(unique_rule,
