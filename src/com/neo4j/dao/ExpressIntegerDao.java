package com.neo4j.dao;

import java.util.ArrayList;
import java.util.List;
import java.util.Map;

import com.type.datatype.ExpressInteger;
import com.type.instance.IntegerInstance;
import com.type.instance.GeneralizedInstance;

public class ExpressIntegerDao extends BaseDao {
	
	/**
	 * 进行图遍历，查询所有的ExpressInteger
	 * @return
	 */
	public List<ExpressInteger> getAllExpressInteger() {
		List<ExpressInteger> res = new ArrayList<ExpressInteger>();
		
		/* 返回integer_type对应的id */
		String sql = "start n=node(*) match (n:Node) where n.name='integer_type' return ID(n) as id";
		List<Map<String, Object>> nodeList = this.getNeoConn().queryList(sql);
		
		/* 每个integer_type创建ExpressInteger */
		for(int i=0; i<nodeList.size(); i++) {
			int integer_type = (Integer) nodeList.get(i).get("id");
			
			res.add(getExpressInteger(integer_type));
		}
		
		return res;
	}
	
	/**
	 * 根据指定的integer节点，解析integer
	 * @param integer_type
	 * @return
	 */
	public ExpressInteger getExpressInteger(Integer integer_type) {
		return new ExpressInteger(integer_type);
	}
	
	/**
	 * 遍历图并寻找IntegerInstance
	 * @return
	 */
	public List<IntegerInstance> getAllExpressIntegerInstances() {
		List<IntegerInstance> intInstances = new ArrayList<IntegerInstance>();
		
		/* 返回explicit_attr对应的id */
		String sql = "start n=node(*) match (n:Node) where n.name='explicit_attr' return ID(n) as id";
		List<Map<String, Object>> explicit_attrs = this.getNeoConn().queryList(sql);
		
		/* 解析每一个explicit_attr下的实例 */
		for (int i = 0; i < explicit_attrs.size(); i++) {
			List<GeneralizedInstance> intInstance = getSimpleDataTypeInstance( (Integer)explicit_attrs.get(i).get("id") );
			for (int j = 0; j < intInstance.size() ; j++) {
				intInstances.add( (IntegerInstance)intInstance.get(j)  );
			}
		}
		
		return intInstances;
	}
	
	/**
	 * 根据指定的explicit_attr节点，解析integer instance
	 * @param explicit_attr
	 * @return
	 */
	public List<GeneralizedInstance> getExpressIntegerInstance(Integer explicit_attr) {
		
		 return getSimpleDataTypeInstance( explicit_attr );
	}
	
	
	public static void main(String[] args) {
		ExpressIntegerDao ins = new ExpressIntegerDao();

		// test ExpressStringInstance
		List<GeneralizedInstance> boolIns = ins.getExpressIntegerInstance(119);
		System.out.println(boolIns);

		/*strIns = ins.getExpressStringInstance(72);
		System.out.println(strIns);
		strIns = ins.getExpressStringInstance(62);
		System.out.println(strIns);*/
//		ExpressString str = ins.getExpressString(79);
//		System.out.println(str);
//		List<ExpressString> strList = ins.getAllExpressString();
//		System.out.println(strList);
		
//		System.out.println(ins.getAllExpressEntity());
//		System.out.println(ins.getExpressRealInstance());
//		System.out.println(ins.getExpressEntity(133));
//		System.out.println(ins.getSimpleDataTypeInstance(149));
//		System.out.println(ins.getVariables(149));
//		List<ExpressReal> realList = ins.getExpressReal();
//		System.out.println(realList);
//		ins.setLocation(83, 20,21,12,13);
//		List<Double> p =  ins.getLocation(195);
//		System.out.println(p);
//		ins.detachDelete(191);
		
//		ins.setProperty(197, "ignore", false);
		ins.logout();

	}
}
